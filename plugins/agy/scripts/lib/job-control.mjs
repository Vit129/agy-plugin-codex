...
function isProgressBlockTitle(line) {
  return ["Final output", "Starting agy"].includes(line);
}
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(
      `Job ${active.id} is still ${active.status}. Check /agy:status and try again once it finishes.`
    );
  }

  if (reference) {
    throw new Error(
      `No finished job found for "${reference}". Run /agy:status to inspect active jobs.`
    );
  }
